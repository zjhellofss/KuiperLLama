...
TEST(test_matmul_cu, matmul_linear_cutlass) {
#ifdef USE_CUTLASS
  LOG(INFO) << "USE_CUTLASS";
#else
  LOG(INFO) << "NOT USE_CUTLASS";
#endif
  auto alloc_cu = base::CUDADeviceAllocatorFactory::get_instance();
  auto alloc_cpu = base::CPUDeviceAllocatorFactory::get_instance();

  tensor::Tensor input(base::DataType::kDataTypeFp32, 4, true, alloc_cpu);
  tensor::Tensor weight(base::DataType::kDataTypeFp32, 8, 4, true, alloc_cpu);
  ...
  tensor::Tensor out_cu(base::DataType::kDataTypeFp32, weight.get_dim(0), true, alloc_cu);
  tensor::Tensor out_cpu(base::DataType::kDataTypeFp32, weight.get_dim(0), true, alloc_cpu);
